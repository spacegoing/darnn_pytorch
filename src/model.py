...
class Decoder(nn.Module):
  ...
  def __init__(self, T, decoder_num_hidden, encoder_num_hidden):
    ...
    self.attn_layer = nn.Sequential(
        nn.Linear(2 * decoder_num_hidden + encoder_num_hidden,
                  encoder_num_hidden), nn.Tanh(),
    ...
    while i < len(y_pred):
      ...
      for j in range(len(batch_idx)):
        if on_train:
          ...
          y_history[j, :] = self.y[range(batch_idx[j],
                                         batch_idx[j] + self.T - 1)]
        else:
          X[j, :, :] = self.X[range(
              batch_idx[j] + self.train_timesteps - self.T, batch_idx[j] +
      ...
      y_pred[i:(i + self.batch_size)] = self.Decoder(
          input_encoded, y_history).cpu().data.numpy()[:, 0]
      i += self.batch_size
